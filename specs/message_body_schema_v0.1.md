...
> **Provenance**: Generated by `cm context "<task>" --json` on {YYYY-MM-DD}. These are **heuristics**, not transcript evidence. If anything conflicts with the excerpt, the excerpt wins.

### Relevant rules (top 3–5)
- [{rule-id}] {one-line rule}  *(or just a bullet if no IDs are available)*

### Known pitfalls / anti-patterns (top 3–5)
- {one-line pitfall to avoid}

### Suggested cass queries (optional)
- `cass search "<query>" --days <n>`

### Prior sessions (optional; default off)
- {very short snippet}  *(include date/workspace if available)*
...
| Memory Context | Optional | If present, includes a clear provenance line and stays brief (avoid crowding out excerpt) |
